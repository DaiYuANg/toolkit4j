...
public class AnimationGraph<T extends AlgebraData<T>> {
  ...
  @Getter private AnimationNode<T> currentNode;
  ...
  AnimationGraph(
      Graph<AnimationNode<T>> graph,
      AnimationApplyFunction<T> apply,
      AnimationStateTransferBeginCallback<T> stateTransferBeginCallback,
    ...
    for (var key : keys) {
      if (key.equals(currentNode)) {
        throw new IllegalArgumentException(
            "node in " + keys + " conflict with current node: " + currentNode);
      }
    }
  ...
  private void playAtNext(
      AnimationNode<T> n, List<AnimationNode<T>> keys, Callback<Void, Exception> cb) {
    if (isReverting()) {
  ...
  private GraphPath<AnimationNode<T>> findShortestPaths(
      AnimationNode<T> from, AnimationNode<T> to, Set<AnimationNode<T>> skip) {
    var paths = graph.shortestPaths(from, skip);
    var ls = paths.get(to);
    if (ls == null) {
      throw new IllegalArgumentException(
          "cannot find path from " + from + " to " + to + ", skip=" + skip);
    }
    return ls;
  }

  private GraphPath<AnimationNode<T>> prepareForPlaying(
      AnimationNode<T> from,
      AnimationNode<T> to,
      GraphPath<AnimationNode<T>> path,
      List<AnimationNode<T>> keys) {
    ...
    public void handle(long now) {
      ...
      if (!isReverting) {
        ...
        if (delta >= currentEdge.durationMillis) {
          ...
          currentEdge = null;
          currentNode.stateTransferFinish.animationStateTransferFinish(
              currentEdge0.from, currentEdge0.to);
          checkEnd();
        } else {
          var p = delta / (double) currentEdge.durationMillis;
          data =
              currentEdge.from.value.plus(
                  currentEdge.to.value.minus(currentEdge.from.value).multiply(p));
        }
      } else {
        ...
        if (delta >= elapsed) {
          ...
          currentEdge = null;
          currentNode.stateTransferFinish.animationStateTransferFinish(
              currentEdge0.to, currentEdge0.from);
          checkEnd();
        } else {
