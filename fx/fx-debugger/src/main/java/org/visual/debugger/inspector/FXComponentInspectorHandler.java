...
public class FXComponentInspectorHandler {
  ...
  private FXComponentInspectorHandler() {
    ...
    pane.getStylesheets()
        .add(
            Objects.requireNonNull(this.getClass().getResource("/component-inspector.css"))
                .toExternalForm());
    ...
    highlightPane
        .getStylesheets()
        .add(
            Objects.requireNonNull(this.getClass().getResource("/highlight.css")).toExternalForm());
  }
  ...
  private void inspectAllWindows() {
    Window.getWindows().stream()
        .filter(
            window ->
                window != popup && window != highlightPopup && !handledWindows.contains(window))
        .forEach(
            window -> {
              handledWindows.add(window);
              window.addEventFilter(
                  MouseEvent.MOUSE_MOVED, event -> handle(event, window.getScene()));
            });
  }
  ...
  private void handle(@NotNull MouseEvent event, Scene scene) {
    ...
    popup.show(
        scene.getRoot(),
        event.getScreenX() - pane.getWidth(),
        event.getScreenY() - pane.getHeight() - OFFSET);
    inspectAllWindows();
  }
