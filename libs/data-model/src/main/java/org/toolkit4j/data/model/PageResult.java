...
public record PageResult<T>(
  Collection<T> content, Integer page, Integer size, Long totalElements, Long totalPages) {
  ...
  @Contract(" -> new")
  public @NonNull PageResult<T> normalized() {
    return new PageResult<>(
      ofNullable(content).orElseGet(ArrayList::new),
      ofNullable(page).orElse(1),
  ...
  public static <T> @NotNull PageResult<T> empty() {
    return new PageResult<>(new ArrayList<>(), 0, 0, 0L, 0L);
  }
}