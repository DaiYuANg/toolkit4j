...
/**
 * 表实现性能基准：HashTable vs ConcurrentHashTable
 * Table benchmark: HashTable vs ConcurrentHashTable
 */
...
public class TableBenchmark {
  ...
  public Table<String, String, Integer> concurrentHashTable_put_parallel() {
    ...
    range(0, size)
        .parallel()
        .forEach(i -> table.put("r" + (i % 100), "c" + i, i));
    return table;
  }
  ...
  public void concurrentHashTable_get_parallel(Blackhole bh, ConcurrentHashTableState state) {
    range(0, size)
        .parallel()
        .forEach(i -> bh.consume(state.table.get("r" + (i % 100), "c" + i)));
  }
