...
public class Trees {
   ...
   */
  public static <T extends FlatNode<ID>, ID> ListTree<T> build(@NonNull Collection<T> flat) {
    return build(flat, null);
  }

  /**
   * 从扁平列表构建树，使用提取器（无需实现 FlatNode）。
   * Build tree using extractors when FlatNode is not implemented.
   *
   * @param flat      扁平列表
   * @param id        id 提取器
   * @param parentId  parentId 提取器
   */
  public static <T, ID> ListTree<T> build(
   ...
   */
  public static <T, ID> ListTree<T> buildList(
      @NonNull Collection<T> flat,
      @NonNull Function<T, ID> id,
      @NonNull Function<T, ID> parentId,
      Comparator<? super T> siblingSort
    ...
    val rootsStream = flat.stream()
        .filter(n -> {
          val pid = parentId.apply(n);
          return pid == null || !nodeMap.containsKey(pid);
        })
        .map(n -> buildNode(n, id, childrenMap, siblingSort));
    val roots = siblingSort == null ? rootsStream.toList() : rootsStream
        .sorted(Comparator.comparing(TreeNode::data, siblingSort))
        .toList();
  ...
  /**
   * 构建 SetTree（根与子节点为 Set，无顺序）。
   * Build SetTree with Set roots and children.
   */
  public static <T extends FlatNode<ID>, ID> SetTree<T> buildSet(@NonNull Collection<T> flat) {
   ...
   * 构建 SetTree，使用提取器。
   */
  public static <T, ID> SetTree<T> buildSet(
      @NonNull Collection<T> flat,
      @NonNull Function<T, ID> id,
      @NonNull Function<T, ID> parentId
  ) {
  ...
  private static <T, ID> TreeNode<T> buildSetNode(
      T data,
      Function<T, ID> id,
      Map<ID, Set<T>> childrenMap
  ) {
    val childSet = childrenMap.getOrDefault(id.apply(data), Set.of());
    val children = childSet.stream()
        .map(c -> buildSetNode(c, id, childrenMap))
        .collect(Collectors.toSet());
    return new SetTreeNodeImpl<>(data, children);
  }

  /**
   * 构建 LinkedTree（根与子节点为 LinkedHashSet，保持插入顺序）。
   * Build LinkedTree with LinkedHashSet, preserves insertion order.
   */
  public static <T extends FlatNode<ID>, ID> LinkedTree<T> buildLinked(@NonNull Collection<T> flat) {
    return buildLinked(flat, FlatNode::id, FlatNode::parentId);
  }

  public static <T, ID> LinkedTree<T> buildLinked(
      @NonNull Collection<T> flat,
      @NonNull Function<T, ID> id,
      @NonNull Function<T, ID> parentId
  ) {
  ...
  private static <T, ID> TreeNode<T> buildLinkedNode(
      T data,
      Function<T, ID> id,
  ...
  /**
   * 构建 SortedTree（根与子节点按 comparator 排序）。
   * Build SortedTree with roots and children sorted by comparator.
   */
  public static <T extends FlatNode<ID>, ID> SortedTree<T> buildSorted(
      @NonNull Collection<T> flat,
  ...
  public static <T, ID> SortedTree<T> buildSorted(
      @NonNull Collection<T> flat,
      @NonNull Function<T, ID> id,
      @NonNull Function<T, ID> parentId,
      @NonNull Comparator<? super T> comparator
  ) {
    if (flat.isEmpty()) return new SortedTreeImpl<>(new TreeSet<>(Comparator.comparing((TreeNode<T> n) -> n.data(), comparator)));
    ...
    val roots = flat.stream()
        ...
        .collect(Collectors.toCollection(() -> new TreeSet<>(nodeComparator)));

    return new SortedTreeImpl<>(roots);
  }

  private static <T, ID> TreeNode<T> buildSortedNode(
      T data,
      Function<T, ID> id,
      Map<ID, List<T>> childrenMap,
      Comparator<? super T> comparator
  ) {
    val childList = childrenMap.getOrDefault(id.apply(data), List.<T>of()).stream()
        .sorted(comparator)
        .toList();
    val nodeComparator = Comparator.<TreeNode<T>, T>comparing(TreeNode::data, comparator);
    val children = childList.stream()
  ...
  /**
   * 从扁平列表构建树，兄弟节点按 comparator 排序。
   * Build tree with siblings sorted by comparator.
   *
   * @param flat        扁平节点列表
   * @param siblingSort 兄弟排序，可为 null
   */
  public static <T extends FlatNode<ID>, ID> ListTree<T> build(
      @NonNull Collection<T> flat,
      Comparator<? super T> siblingSort
  ) {
  ...
  private static <T, ID> TreeNode<T> buildNode(
      T data,
      Function<T, ID> id,
      Map<ID, List<T>> childrenMap,
      Comparator<? super T> siblingSort
    ...
    val children = sorted.stream()
        .map(c -> buildNode(c, id, childrenMap, siblingSort))
        .toList();
