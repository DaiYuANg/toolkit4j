...
public class DistributedNodeUtil {
  ...
  /**
   * 节点 ID 最大值（与 {@link org.agrona.concurrent.SnowflakeIdGenerator#NODE_ID_BITS_DEFAULT} 一致：10 bit →
   * 0..1023）
   */
   ...
   *
   * <ol>
   *   <li>根据本机主机名生成节点 ID（hash 后取模）
   *   <li>解析失败时使用 {@link #DEFAULT_NODE_ID}
   * </ol>
   *
   * <p>注意：不同主机可能映射到同一 ID，生产环境建议显式配置 {@code snow.flake.generator.node-id}。
   *
   * @return 节点 ID
   */
  public long getNodeId() {
    ...
    if (hostname == null || hostname.isBlank()) {
      log.debug(
          "Falling back to default snowflake nodeId={} because local hostname is unavailable",
          DEFAULT_NODE_ID);
   ...
   *
   * <p>使用无符号方式处理 {@link String#hashCode()}，避免 {@link Math#abs(int)} 对 {@link Integer#MIN_VALUE}
   * 的行为及负数取模问题。
   *
   * @param hostname 主机名
